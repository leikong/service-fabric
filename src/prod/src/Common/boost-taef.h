...
inline static boost::unit_test::const_string BoostTaef_UnicodeToAnsi(CHAR const * ws = nullptr)
{
    if (ws == nullptr) return boost::unit_test::const_string();

#ifdef PLATFORM_UNIX
    std::string ansiString = std::string_convert<std::codecvt_utf8_utf16<char16_t>, char16_t>().to_bytes((const char16_t*)ws);
#else
    std::string ansiString = std::string_convert<std::codecvt_utf8_utf16<unsigned short>, unsigned short>().to_bytes((const unsigned short*)ws);
#endif
...
#define VERIFY_ARE_EQUAL2(a, b) { auto vl = (a); auto vr = (b); VERIFY_IS_TRUE(vl == vr, Common::formatString.L("VERIFY_ARE_EQUAL({0}={1}, {2}={3})", #a, vl, #b, vr).c_str()); }

#define VERIFY_ARE_NOT_EQUAL(a, b, ...) VERIFY_IS_TRUE((a) != (b), __VA_ARGS__)
#define VERIFY_ARE_NOT_EQUAL2(a, b) { auto vl = (a); auto vr = (b); VERIFY_IS_TRUE(vl != vr, Common::formatString.L("VERIFY_ARE_NOT_EQUAL({0}={1}, {2}={3})", #a, vl, #b, vr).c_str()); }
...
{\
    auto msg = BoostTaef_UnicodeToAnsi(formatString(__VA_ARGS__).c_str()); \
    BOOST_TEST_MESSAGE(msg);\
}

#define VERIFY_IS_TRUE_FMT(condition, ...) VERIFY_IS_TRUE(condition, Common::formatString(__VA_ARGS__).c_str())
#define VERIFY_IS_FALSE_FMT(condition, ...) VERIFY_IS_FALSE(condition, Common::formatString(__VA_ARGS__).c_str())
#define VERIFY_ARE_EQUAL_FMT(a, b, ...) VERIFY_IS_TRUE((a) == (b), Common::formatString(__VA_ARGS__).c_str())
#define VERIFY_FAIL_FMT(...) VERIFY_FAIL(Common::formatString(__VA_ARGS__).c_str())
